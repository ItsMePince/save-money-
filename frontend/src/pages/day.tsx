    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                if (!resExpenses.ok) throw new Error(`โหลดรายการไม่สำเร็จ (${resExpenses.status})`);
                if (!resRepeated.ok) throw new Error(`โหลดรายการซ้ำไม่สำเร็จ (${resRepeated.status})`);

                const expensesData: ExpenseDTO[] = await resExpenses.json();
                const repeatedData: ApiRepeatedTransaction[] = await resRepeated.json();
                if (!alive) return;

                const filteredRepeated = repeatedData.filter(rt => {
                    const isoDate = toISODate(rt.date);
                    return isoDate === dayISO;
                });

                const repeatedDTOs: ExpenseDTO[] = filteredRepeated.map(mapRepeatedToExpenseDTO);

                setRaw([...expensesData, ...repeatedDTOs]);
