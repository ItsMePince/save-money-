...
describe('More Page', () => {
  ...
  it('Export CSV เมื่อมีข้อมูล', () => {
    ...
    cy.window().then((win) => {
      cy.stub(win, 'URL').returns({
        createObjectURL: cy.stub().returns('blob:mock-url'),
        revokeObjectURL: cy.stub()
      });
    });
  ...
  it('แสดง alert เมื่อ Export ล้มเหลว', () => {
    ...
    cy.contains('Export CSV').click();
    cy.wait('@getError');
    cy.get('@alertStub').should('be.calledWith', 'Export ข้อมูลไม่สำเร็จ ❌ (ไม่สามารถเชื่อมต่อ API ได้)');
  });
